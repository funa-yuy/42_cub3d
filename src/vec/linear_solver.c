...
/// ]
/// ```
static t_line_segment matrix_2x2_mapper(t_f32x4 p, t_f32x4 q, t_f32x4 r , int offset)
{
	t_vec_f32x4 rs;
	...
	if (offset == 0)
		return ((t_line_segment){
			.s = add_f32x4(mul_f32x4(p, init_f32x4(0, 0, 1, 0)),	
				init_f32x4(0, rs.x, 0, 0)),
			.e = add_f32x4(mul_f32x4(q, init_f32x4(0, 0, 1, 0)),	
				init_f32x4(0, rs.y, 0, 0))});
	else
		return ((t_line_segment){
			.s = add_f32x4(mul_f32x4(p, init_f32x4(0, 1, 0, 0)),	
					init_f32x4(0, 0, rs.x, 0)) ,
			.e = add_f32x4(mul_f32x4(q , init_f32x4(0, 1, 0, 0)),	
...
)
{
	...
	return (init_f32x4(
			0,
		       	...
		       	matrix_scalar_2x2(y_nume.s, y_nume.e) / base,
			0));
}
